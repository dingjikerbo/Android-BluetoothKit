...
这个库用于Android蓝牙BLE设备通信，支持设备扫描，连接，读写，通知，可同时管理多个BLE蓝牙设备的连接，并解决了BLE蓝牙通信中可能会遇到的一系列坑，使得Android蓝牙开发非常方便。
...
四、统一管理连接句柄，避免句柄泄露 
五、方便监控各设备连接状态，比如手机可同时连接的设备数有限，可以检测设备连接活跃度，在尽可能维持连接的情况下，将最不活跃的设备自动断开。 
六、支持多进程APP架构下蓝牙连接的管理 
七、支持拦截所有对蓝牙原生接口的调用
